...
def extract_stationary_structures(prefix, Opt=True,increment=10):
    ...
    for filestem in range(-180,180,increment):
        ...
        if os.path.isfile( file ):
            ...
            print(file)
            for linenumber in range(len(filecontent)-1):

                if '\\HF='  in  filecontent[linenumber].replace("\n", "")+filecontent[linenumber+1].replace(" ", ""):
