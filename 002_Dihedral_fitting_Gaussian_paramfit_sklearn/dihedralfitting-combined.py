...
def comp_with_derivdata3( outputparams):
        ...
        #print(angle )
        yval=np.zeros(len(outputparams[0][1]))
...
def fctderiv2min_combined(params, inputfct,data,dataderiv, weights=False):
    ...
    # Solution1 : mdtraj.computedihedrals  : in rad :-)
    # Solution2 : read angle harmonic equilibration values from frcmod  : in degrees --> conversion
    modeldata = np.zeros(len(inputfct[0][1]))
    sum_amp=0
    modelderiv = np.zeros(len(inputfct[0][1]))
    for inputdih in inputfct:
        ampparam=inputdih[2]
        x= inputdih[1]
        order= inputdih[3]
        ...
        sum_amp+=amp_abs
        modeldata +=   np.multiply(   1  +  np.cos(np.multiply(x,order ) + phase  ) ,   amp_abs)
    modeldata += params['Kdata']
    ...
    for inputdih in inputfct:
        ...
        order= inputdih[3]
        amp = params[ ampparam].value
        modelderiv +=   np.multiply(      np.sin(np.multiply(x,order )           ) , -  amp * order)
    modelderiv += params['Kderiv']
    if weights==False :
        residus =np.concatenate( (modeldata - data ,modelderiv - dataderiv)  )
        #residus =np.concatenate( (modelderiv - dataderiv,modelderiv - dataderiv )  )
        return residus

    else :

        residus =np.concatenate( ( modeldata - data ,modelderiv - dataderiv))
        return np.multiply(residus, weights)


def fctderiv2min_simple(params, inputfct,data,weights=False):
...
def fctderiv2min_jacobian(params, inputfct):
    ...
    for inputdih in inputfct:
        ...
        order= inputdih[3]
        amp = params[ ampparam].value
        Jac.append(   np.multiply(      np.sin(np.multiply(x,order )           ) , -  amp * order))
...
def leastsquare_with_prem(x,qm,dih_idx, dih_type, maxorder=1 , prem_data=[[],[]]):
    ...
    print(weights)
    result  = lmfit.minimize( fctderiv2min_combined, params, args=(inputfct, qm, qmderiv ,weights) , method = 'cg')
    return result ,inputfct
    # calculate final result

#result ,inputfct=leastsquare_with_prem(x.T,derivdata, all_dihedrals_type_index[torsion],all_dihedrals_type[torsion], 1)

def leastsquare_with_prem_combined(x,qm , qmderiv,dih_idx, dih_type, maxorder=1 , prem_data=[[],[]]):

    #n_dih=len(dih_idx)
    dih_type_index =  [ j[-1]  for j in dih_idx ]
    #print(len(dih_type_index))
    if len(dih_type_index) == 9 :
        orders = [ [1,2,3,4] for x in range(len(dih_type_index))]
        amps=[[0,0,0,0] for x in range(len(dih_type_index))]
    if len(dih_type_index) != 9 :
        orders = [ [1,2,4,5] for x in range(len(dih_type_index))]
        amps=[[0,0,0,0] for x in range(len(dih_type_index))]
        maxorder+=1
    print(orders, amps)
    #orders, amps = guess_order('gaff2.frcmod',dih_type)
    ...
    for j in range (len(dih_idx)):
            ...
            if typedih in impropers_propers :
                ...
                params.add('ampIMP_%s'%(name), value=0 )
            '''
    params.add('Kdata', value=0 , max=10000 , min=-10000)
    params.add('Kderiv', value=0 , max=10000 , min=-10000)
    ...
    method = 'L-BFGS-B'
    reduce_fcn='neglogcauchy'
    #minner = Minimizer(fctderiv2min_combined, params, fcn_args=(inputfct, qm, qmderiv))
    #result = minner.minimize(method = 'L-BFGS-B')
    print(qm)
    weights=[ max ( 0.95 , min( 1/math.sqrt(i+0.01) , 1.05) ) for i in  qmderiv  ]
    print(weights)
    weights.extend([ 0.5 for i in qmderiv  ])
    #result  = lmfit.minimize( fctderiv2min_combined, params, args=(inputfct, qm, qmderiv, weights) , method = 'basinhopping')
    #result  = lmfit.minimize( fctderiv2min_combined, params, args=(inputfct, qm, qmderiv, weights), method = 'lbfgsb' ,  reduce_fcn='neglogcauchy'  )
    #result  = lmfit.minimize( fctderiv2min_combined, params, args=(inputfct, qm, qmderiv) , method = 'leastsq'  )
    #result  = lmfit.minimize( fctderiv2min_combined, params, args=(inputfct, qm,qmderiv, weights ) , method = 'differential_evolution'  )

    #result  = lmfit.minimize( fctderiv2min_combined, params, args=(inputfct, qm, qmderiv, weights) , method = 'ampgo')
    #result  = lmfit.minimize( fctderiv2min_combined, params, args=(inputfct, qm, qmderiv, weights) , method = 'nelder')
    result  = lmfit.minimize( fctderiv2min_combined, params, args=(inputfct, qm, qmderiv, weights ) , method = 'cg')
    print(qm)
    #lambda x:
    #result  = lmfit.minimize( fctderiv2min_simple, params, args=(inputfct, qm ) , method = 'trust-ncg',  jac= fctderiv2min_jacobian)
    #result  = lmfit.minimize( fctderiv2min_combined, params, args=(inputfct, qm, qmderiv, weights) , method = 'cobyla')
    #result  = lmfit.minimize( fctderiv2min_combined, params, args=(inputfct, qm, qmderiv, weights) , method = 'tnc')

    #result  = lmfit.minimize( fctderiv2min_combined, params, args=(inputfct, qm, qmderiv, weights) , method = 'dual_annealing')
...
def add_new_parms(inputfile,newfrcmod):
    ...
    tleapinput.writelines( [ 'new = loadmol2  %s  \n' %(inputfile),   'saveAmberParm  new  newtopol.prmtop new.rst7d\n ' ] )
...
if __name__ =='__main__':
        ...
        parser.add_argument("-resname", type=str, default='5', help="name od the residue")
        ...
        for torsion in range(len(torsion_names)):
            if  torsion_names[torsion]!=False:
                    ...
                    if energy.shape[0] == len(range(-180,180,10))  and max(energy) <100 :
                        ...
                        if adjust < 6 :   # how many times can we cheat ?
                            ...
                            if len(energies_torsions) == 0 :
                                    ...
                                    plt.plot(range(-180,180,10),energies_torsions )

                            else :  energies_torsions=energies_torsions + energy
                        else :  conf_per_torsion[torsion ]=-1
                    else : conf_per_torsion[torsion ]=-1
                    plt.savefig('energyresidual%s'%(torsion_names[torsion ]))
                    data_all.append(energy)
                plt.close()
                if  conf_per_torsion[torsion ]==0 :sys.exit('no valid data for torsion' + torsion_names[torsion] )
                datavg=energies_torsions/conf_per_torsion[torsion ]

                outputenergy=open('%s-residual-energySP-avg.dat' %(torsion_names[torsion]) ,'w')
                outputenergy.writelines( [ '%.8f \n' %e for e in  datavg ])
                outputenergy.close()
                outputenergy=open('%s-residual-energySP.dat' %(torsion_names[torsion]) ,'w')
                outputenergy.writelines( [ '%.8f \n' %e for e in  data])
                outputenergy.close()
                ...
                #print(len(data))
                #print(len(x.T[0]))
                print(len(data),len(datavg), len(x.T[0]) )

                #Workwith AVERAGES or not comment/uncomment
                '''
                datavg,adjust = smooth(datavg)
                derivdatavg=np.gradient(datavg,x.T[0])
                derivdatavg[0]=(datavg[1]-datavg[-1])/2
                derivdatavg[-1]=(datavg[-2]-datavg[0])/2
                derivdata=derivdatavg
                data=datavg
                '''
                derivdata_all=[]
                for data in data_all :
                    ...
                    derivdata[0]=(data[1]-data[-1])/2
                    derivdata[-1]=(data[-2]-data[0])/2
                    #'''
                    k=np.min(data)
                    data=data-k
                    k=np.min(derivdata)
                    derivdata=derivdata-k
                    derivdata_all.append(derivdata)
                outputenergy=open('%s-residual-energySP-avg-derived.dat' %(torsion_names[torsion]) ,'w')
                outputenergy.writelines( [ '%.8f \n' %e for e in  derivdata ])
                outputenergy.close()
                #print(data)



                result ,inputfct=leastsquare_with_prem_combined(x.T,data , derivdata, all_dihedrals_type_index[torsion],all_dihedrals_type[torsion], maxorder)
                ...
                #print( all_dihedrals_type)
                paramsout.append(write_frcmod(result,torsion_names[torsion],  all_dihedrals_type,all_dihedrals_type_index))

                #xvalues, fitteddata=comp_with_derivdata2(paramsout[-1],inputfct)
                #plot(derivdata,fitteddata,  xvalues )
                outputfct=[]
                ...
                #plot( derivdata, derivdata + result.residual[36:],xvalues, outputfile=torsion_names[torsion]+'derivedfunction-fit-residuals.png' )
                plot(data,data + result.residual[:36],  xvalues , outputfile=torsion_names[torsion]+'primaryfunction-fit-residuals.png')
